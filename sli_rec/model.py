    ...
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        with tf.name_scope('Inputs'):
            self.item_history = tf.placeholder(
                tf.int32, [None, None], name='item_history')
            self.cate_history = tf.placeholder(
                tf.int32, [None, None], name='cate_history')
            self.timeinterval_history = tf.placeholder(
                tf.float32, [None, None], name='timeinterval_history')
            self.timelast_history = tf.placeholder(
                tf.float32, [None, None], name='timelast_history')
            self.timenow_history = tf.placeholder(
                tf.float32, [None, None], name='timenow_history')
            self.user = tf.placeholder(tf.int32, [None, ], name='user')
            self.targetitem = tf.placeholder(
                tf.int32, [None, ], name='targetitem')
            self.targetcate = tf.placeholder(
                tf.int32, [None, ], name='targetcate')
        ...
        with tf.name_scope('Embedding_layer'):
            self.item_lookup = tf.get_variable(
                "item_lookup", [item_number, EMBEDDING_DIM])
            tf.summary.histogram('item_lookup', self.item_lookup)
            self.targetitem_embedding = tf.nn.embedding_lookup(
                self.item_lookup, self.targetitem)
            self.itemhistory_embedding = tf.nn.embedding_lookup(
                self.item_lookup, self.item_history)

            self.cate_lookup = tf.get_variable(
                "cate_lookup", [cate_number, EMBEDDING_DIM])
            tf.summary.histogram('cate_lookup', self.cate_lookup)
            self.targetcate_embedding = tf.nn.embedding_lookup(
                self.cate_lookup, self.targetcate)
            self.catehistory_embedding = tf.nn.embedding_lookup(
                self.cate_lookup, self.cate_history)

        self.target_item_embedding = tf.concat(
            [self.targetitem_embedding, self.targetcate_embedding], 1)
        self.item_history_embedding = tf.concat(
            [self.itemhistory_embedding, self.catehistory_embedding], 2)
...
class Model_ASVD(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_ASVD, self).__init__(user_number, item_number,
                                         cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)

        # Sum
        self.item_history_embedding_sum = tf.reduce_sum(
            self.item_history_embedding, 1)
        last_inps = tf.concat(
            [self.target_item_embedding, self.item_history_embedding_sum], 1)
        self.fcn_net(last_inps, use_dice=False)


class Model_DIN(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_DIN, self).__init__(user_number, item_number,
                                        cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)

        # Attention layer
        with tf.name_scope('Attention_layer'):
            attention_output = attention_DIN(
                self.target_item_embedding, self.item_history_embedding, ATTENTION_SIZE, self.mask)
...
class Model_LSTM(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_LSTM, self).__init__(user_number, item_number,
                                         cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_LSTMPP(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_LSTMPP, self).__init__(user_number, item_number,
                                           cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)

        # Attention layer
        with tf.name_scope('Attention_layer'):
            att_outputs, alphas = attention_HAN(
                self.item_history_embedding, attention_size=ATTENTION_SIZE, return_alphas=True)
        ...
        with tf.name_scope('User_alpha'):
            concat_all = tf.concat([self.target_item_embedding, att_fea, final_state[1], tf.expand_dims(
                self.timenow_history[:, -1], -1)], 1)
            concat_att1 = tf.layers.dense(
                concat_all, 80, activation=tf.nn.sigmoid, name='concat_att1')
            concat_att2 = tf.layers.dense(
                concat_att1, 40, activation=tf.nn.sigmoid, name='concat_att2')
            user_alpha = tf.layers.dense(
                concat_att2, 1, activation=tf.nn.sigmoid, name='concat_att3')
            user_embed = att_fea * user_alpha + \
                final_state[1] * (1.0 - user_alpha)
...
class Model_NARM(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_NARM, self).__init__(user_number, item_number,
                                         cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_CARNN(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_CARNN, self).__init__(user_number, item_number,
                                          cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
        self.item_history_embedding = tf.concat(
            [self.item_history_embedding, tf.expand_dims(self.timeinterval_history, -1)], -1)
...
class Model_Time1LSTM(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_Time1LSTM, self).__init__(user_number, item_number,
                                              cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_Time2LSTM(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_Time2LSTM, self).__init__(user_number, item_number,
                                              cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_Time3LSTM(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_Time3LSTM, self).__init__(user_number, item_number,
                                              cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_DIEN(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_DIEN, self).__init__(user_number, item_number,
                                         cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_A2SVD(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_A2SVD, self).__init__(user_number, item_number,
                                          cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_T_SeqRec(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_T_SeqRec, self).__init__(user_number, item_number,
                                             cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)

        item_history_embedding_new = tf.concat(
            [self.item_history_embedding, tf.expand_dims(self.timelast_history, -1)], -1)
        item_history_embedding_new = tf.concat(
            [item_history_embedding_new, tf.expand_dims(self.timenow_history, -1)], -1)
...
class Model_TC_SeqRec_I(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_TC_SeqRec_I, self).__init__(user_number, item_number,
                                                cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_TC_SeqRec_G(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_TC_SeqRec_G, self).__init__(user_number, item_number,
                                                cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_TC_SeqRec(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_TC_SeqRec, self).__init__(user_number, item_number,
                                              cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_SLi_Rec_Fixed(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_SLi_Rec_Fixed, self).__init__(user_number, item_number,
                                                  cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
...
class Model_SLi_Rec_Adaptive(Model):
    def __init__(self, user_number, item_number, cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE):
        super(Model_SLi_Rec_Adaptive, self).__init__(user_number, item_number,
                                                     cate_number, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE)
        ...
        with tf.name_scope('CNN_layer'):
            ...
            with open('/content/fair2023/sli_rec/cnn.txt', 'w') as writefile:
                ...
                writefile.write(str(tf.shape(self.item_history_embedding)))
            '''
            #Padding CNN input
            ...
            cnn_features = tf.layers.dense(
                cnn_features, 36, activation=tf.nn.relu, name='cnn_features2')
            # print("Shape of cnn_features:", tf.shape(cnn_features))
            tf.summary.histogram('cnn_features', cnn_features)
        ...
        # Attention layer(3)
        with tf.name_scope('Attention_layer_3'):
            att_outputs3, alphas3 = attention_FCN2(self.target_item_embedding, cnn_features, ATTENTION_SIZE, self.mask,
                                                   softmax_stag=1, stag='1_1', mode='LIST', return_alphas=True, forCnn=True,  scope='1')
            tf.summary.histogram('alpha_outputs3', alphas3)
            att_fea3 = tf.reduce_sum(att_outputs3, 1)
            tf.summary.histogram('att_fea2', att_fea3)
        ...
        with tf.name_scope('User_alpha'):
            ...
            concat_att2 = tf.layers.dense(
                concat_att1, 40, activation=tf.nn.sigmoid, name='concat_att2')
            # Change user_alpha computation
            user_alpha_logits = tf.layers.dense(
                concat_att2, 3, activation=None, name='user_alpha_logits')
            user_alpha = tf.exp(
                user_alpha_logits - tf.reduce_max(user_alpha_logits, axis=1, keep_dims=True))
            user_alpha /= tf.reduce_sum(user_alpha, axis=1, keep_dims=True)
