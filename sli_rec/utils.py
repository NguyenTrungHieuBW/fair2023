...
class Time4AILSTMCell(RNNCell):
    ...
    def call(self, inputs, state, att_score=None):
        ...
        time_now_state = math_ops.matmul(inputs, self._time_kernel_w1) + math_ops.matmul(
            time_now_input, self._time_kernel_t1) + self._time_bias1
        time_last_state = math_ops.matmul(inputs, self._time_kernel_w2) + math_ops.matmul(
            time_last_input, self._time_kernel_t2) + self._time_bias2
        ...
        if self._use_peepholes:
            ...
            c = (sigmoid(f + self._forget_bias) * sigmoid(time_last_state) *
...
def attention_DIN(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False):
    ...
    d_layer_1_all = tf.layers.dense(
        din_all, 80, activation=tf.nn.sigmoid, name='f1_att' + stag)
    d_layer_2_all = tf.layers.dense(
        d_layer_1_all, 40, activation=tf.nn.sigmoid, name='f2_att' + stag)
...
def attention_FCN(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False, forCnn=False, scope=""):
    ...
    query = tf.layers.dense(
        query, facts_size, activation=None, name='f1' + stag)
